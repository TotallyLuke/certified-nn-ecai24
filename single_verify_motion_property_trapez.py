...
def verify_motion_property_2barrier(model: nn.Module, model2, turn_index, controller, psi_deg_init, interv_width = 3.0, verbose=False):
    turn = ACTIONS[turn_index]
    ...
    m = milp_encoding(model, K=1, input_bounds=[current_state_bounds], NN_Inputs=NN_Inputs, NN_Outputs=NN_Outputs)
    m = milp_encoding(model2, K=1, input_bounds=[next_state_bounds], NN_Inputs=NN_Inputs,
                      NN_Outputs=NN_Outputs, var_offset=1, gurobi_model=m)
    if verbose:
        summarize_model(m)
